...
def gen_aug(sample, ssh_type):
    if ssh_type == 'na':
        ...
        scale_sample = scaling(jitter(sample), sigma=0.5)
        return torch.from_numpy(scale_sample)
    elif ssh_type == 'perm_jit':
        return jitter(permutation(sample, max_segments=10), sigma=0.5)
        ...
        return random_time_shift(sample)
        ...
        return gen_new_aug_3_ablation(sample, None, None, None)
    else:

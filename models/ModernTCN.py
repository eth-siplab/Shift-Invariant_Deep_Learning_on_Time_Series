...
class ModernTCN(nn.Module):
    def __init__(self, args, task_name='classification', patch_size=5, patch_stride=2, stem_ratio=1, downsample_ratio=1, ffn_ratio=1, num_blocks=[1, 1], large_size=[31, 31], small_size=[5, 5], dims=[24, 24], dw_dims=[64, 64],
                 nvars=1, small_kernel_merged=False, backbone_dropout=0.1, head_dropout=0.1, use_multi_scale=True, revin=True, affine=True,
                 subtract_last=False, freq=None, seq_len=200, c_in=6, individual=False, target_window=32, class_drop=0., class_num = 10):

        super(ModernTCN, self).__init__()

        # UCIHAR -- > committed, nvars = 9
        # IEEE Big --> nvars = 1
        ...
        if use_multi_scale:
            self.head_nf = d_model * patch_num # --> 24 * 64

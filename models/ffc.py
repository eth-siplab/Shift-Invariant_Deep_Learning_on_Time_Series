...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        # (batch, c, h, w/2+1, 2)
        ffted = torch.view_as_real(torch.fft.rfft2(x, dim=2, norm='ortho'))
        # (batch, c, 2, h, w/2+1)
        ...
        output = torch.fft.irfft2(torch.view_as_complex(ffted), s=(h,2), dim=(2,3), norm='ortho')
...
class SpectralTransform(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.enable_lfu:
            ...
            xs = self.lfu(xs)
            xs = xs.repeat(1, 1, split_no, 1).contiguous()
        else:
            xs = 0

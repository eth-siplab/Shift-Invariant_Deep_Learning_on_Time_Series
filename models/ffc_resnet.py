...
class FFCResNet(nn.Module):
    ...
    def __init__(self, layers, n_channels, num_classes=1000, zero_init_residual=False,
        ...
        self.conv0 = nn.Conv1d(n_channels, 3, kernel_size=3, stride=1, padding=1)
        ...
        self.lfu = lfu
        self.use_se = use_se
        self.conv1 = nn.Conv2d(3, inplanes, kernel_size=(5,1), stride=2, padding=2, bias=False)
        ...
        self.layer2 = self._make_layer(
            block, inplanes * 2, layers[1], stride=1, ratio_gin=ratio, ratio_gout=ratio)
        self.layer3 = self._make_layer(
            block, inplanes * 4, layers[2], stride=1, ratio_gin=ratio, ratio_gout=ratio)
        # self.layer4 = self._make_layer(
        #     block, inplanes * 8, layers[3], stride=1, ratio_gin=ratio, ratio_gout=0)
    ...
    def forward(self, x):
        if x.shape[1] == 100:
            x = torch.nn.functional.pad(input=x, pad=(0, 0, 2, 0), mode='constant', value=0)
        x = x.transpose(1,2)

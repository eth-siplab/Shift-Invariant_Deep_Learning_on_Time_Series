'''https://github.com/Shuheng-Li/UniTS-Sensory-Time-Series-Classification/blob/main/source/model/STF.py'''
...
def complex_glorot_uniform(c_in, c_out_total, fft_list, fft_n, use_bias=True, name='complex_mat'):
    ...
    for fft_elem in fft_list:
        if fft_elem != fft_n:
            transforms = torchvision.transforms.Resize((1, int(fft_elem/2)+1), antialias=True )
...
class STFlayer(nn.Module):
    def __init__(self, fft_list, f_step_list, kernel_len_list, dilation_len_list, c_in, c_out,
        ...
        self.patch_kernel_dict, self.patch_bias = complex_glorot_uniform(c_in, c_out, self.fft_n_list, KERNEL_FFT)
    ...
    def forward(self, inputs):
        ...
        for fft_idx, fft_n in enumerate(self.fft_n_list):
            ...
            try:
                ...
                patch_fft = patch_fft.reshape(inputs.shape[0], -1, patch_fft.size(-2), patch_fft.size(-1))
            ...
            for fft_idx2, tar_fft_n in enumerate(self.fft_n_list):
                if tar_fft_n < fft_n:
                    ...
                    time_ratio = int(tar_fft_n / fft_n)
                    patch_fft_mod = torch.reshape(patch_fft, (patch_fft.size(0), -1, time_ratio, patch_fft.size(-2), patch_fft.size(-1) ) )
                    
                    patch_fft_mod = patch_fft_mod.permute(0, 1, 3, 4, 2)
                    patch_fft_mod = atten_merge(patch_fft_mod, self.merge_kernel[fft_idx][fft_idx2].to(patch_fft_mod.device), self.merge_bias[fft_idx][fft_idx2].to(patch_fft_mod.device)) * float(time_ratio)
        ...
        for fft_idx, fft_n in enumerate(self.fft_n_list):
            ...
            if FILTER_FLAG:
                patch_kernel = self.patch_kernel_dict[fft_n].to(patch_fft.device)
                patch_fft = torch.complex(patch_fft_r, patch_fft_i)
        ...
        if FILTER_FLAG:
            patch_time_final = patch_time_final + self.patch_bias.real.to(patch_time_final.device)
...
class STFNet(nn.Module):
    def __init__(self, args):
        ...
        SENSOR_AXIS = args.n_feature
        SERIES_SIZE = args.len_sw
        SERIES_SIZE2 = int(0.75 * args.len_sw)
        GEN_C_OUT = 112
        ...
        for i in range(SENSOR_TYPE):
            ...
            self.layers1.append(STFlayer(GEN_FFT_N, GEN_FFT_STEP, FILTER_LEN, DILATION_LEN,
                         int(args.n_feature / SENSOR_TYPE), GEN_C_OUT, ser_size=SERIES_SIZE, BATCH_SIZE = args.batch_size))
    ...
    def forward(self, inputs):
        ...
        # inputs = torch.nn.functional.pad(inputs, (2,54), 'constant', 1)
        inputs = torch.nn.functional.pad(inputs, (2,126), 'constant', 1)

...
from equiadapt.images_and_timeseries.utils import (
    flip_boxes,
    flip_masks,
...
class DiscreteGroupImageCanonicalization(DiscreteGroupCanonicalization):
    ...
    def canonicalize(self, x: torch.Tensor, targets: Optional[List] = None, **kwargs: Any) -> Union[torch.Tensor, Tuple[torch.Tensor, List]]:
        """
    ...
    def invert_canonicalization(self, x_canonicalized_out: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        """
...
class OptimizedGroupEquivariantImageCanonicalization(DiscreteGroupImageCanonicalization):
    ...
    def rotate_and_maybe_reflect(self, x: torch.Tensor, degrees: torch.Tensor, reflect: bool = False) -> List[torch.Tensor]:
        """
...
class DiscreteGroupSignalCanonicalization(DiscreteGroupCanonicalization):
    """
    This class represents a discrete group canonicalization model for 1D signals.

    The model is designed to be equivariant under discrete translations.
    """
    ...
    def __init__(
        ...
        """
        Initializes the DiscreteGroupSignalCanonicalization instance.

        Args:
            canonicalization_network (torch.nn.Module): The canonicalization network.
            canonicalization_hyperparams: The hyperparameters for the canonicalization process.
            in_shape (tuple): The shape of the input signals.
        """
        super().__init__(canonicalization_network)
        self.num_translations = num_translations
        self.group_type = "translation"
        self.group_info_dict = {
    ...
    def groupactivations_to_groupelement(self, group_activations: torch.Tensor) -> dict:
        """
        Converts group activations to the corresponding translation elements.

        Args:
            group_activations (torch.Tensor): The activations for each translation element.

        Returns:
            dict: The corresponding group elements (translations).
        """
        group_elements_one_hot = self.groupactivations_to_groupelementonehot(group_activations)
        translations = torch.arange(self.num_translations, device=group_activations.device)
        translation_element = torch.sum(group_elements_one_hot * translations, dim=-1)
        return {"translation": translation_element}

    def get_group_activations(self, x: torch.Tensor) -> torch.Tensor:
        """
        Gets the group activations for the input signals.

        Args:
            x (torch.Tensor): The input signals.
    ...
    def get_groupelement(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        Maps the input signal to a group element.
    ...
    def canonicalize(
        ...
        """
        Canonicalizes the input signals.

        Args:
            x (torch.Tensor): The input signals.
            targets (Optional[List], optional): Additional targets that may need canonicalization.

        Returns:
            Union[torch.Tensor, Tuple[torch.Tensor, List]]: The canonicalized signals and optionally the targets.
        """
        self.device = x.device
        group_element_dict = self.get_groupelement(x)

        shift = -group_element_dict["translation"].long()
        x_canonicalized = torch.stack([torch.roll(x[i], shifts=int(shift[i].item()), dims=-2) for i in range(x.size(0))])

        if targets:
            targets = [torch.roll(t, shifts=shift.item(), dims=-1) for t in targets]
            return x_canonicalized, targets
    ...
    def invert_canonicalization(
        ...
        """
        Inverts the canonicalization to recover the original signal orientation.

        Args:
            x_canonicalized_out (torch.Tensor): The canonicalized signal output.

        Returns:
            torch.Tensor: The signal reverted to its original orientation.
        """
        shift = self.canonicalization_info_dict["group_element"]["translation"].long()
        return torch.roll(x_canonicalized_out, shifts=shift.item(), dims=-1)

class GroupEquivariantSignalCanonicalization(DiscreteGroupSignalCanonicalization):
    """
    This class represents a discrete group equivariant canonicalization model for 1D signals.

    The model is designed to be equivariant under discrete translations.

    Methods:
        __init__: Initializes the GroupEquivariantSignalCanonicalization instance.
        get_group_activations: Gets the group activations for the input signals.
    """
    ...
    def __init__(
        ...
        """
        Initializes the GroupEquivariantSignalCanonicalization instance.
        ...
        """
        super().__init__(
            canonicalization_network, num_translations, in_shape
        )

        self.group_type = "translation"
        self.num_translations = num_translations
        self.num_group = (self.num_translations)   
        self.group_info_dict = {
            "num_translations": self.num_translations,
            "num_group": self.num_group,
        }
    ...
    def get_group_activations(self, x: torch.Tensor) -> torch.Tensor:
        ...
        This method takes a signal as input, applies any necessary transformations,
        forwards it through the canonicalization network, and returns the group activations.

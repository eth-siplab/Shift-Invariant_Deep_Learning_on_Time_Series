...
class ESCNN_translation_EquivariantNetwork(torch.nn.Module):
    def __init__(
        self,
        in_shape: tuple,         # (in_channels, signal_length)
        out_channels: int,       # number of channels per group element
        kernel_size: int = 5,
        num_translations: int = 16,  # number of discrete translation elements
        num_layers: int = 3,
    ):
        """
        ...
        super().__init__()
        self.in_channels = in_shape[0]
        self.signal_length = in_shape[1]
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        self.num_translations = num_translations

        # In our design, we want the final channel dimension to be:
        #   out_channels * num_translations
        # so that we can reshape to [batch, out_channels, num_translations].
        layers = []
        # First layer: map from in_channels to out_channels * num_translations
        layers.append(
            torch.nn.Conv1d(self.in_channels, out_channels * num_translations, kernel_size, 
                      padding=kernel_size // 2)
        )
        layers.append(torch.nn.BatchNorm1d(out_channels * num_translations))
        layers.append(torch.nn.ReLU(inplace=True))
        layers.append(torch.nn.Dropout(p=0.5))
        # Add hidden layers if num_layers > 2
        for _ in range(num_layers - 2):
            layers.append(
                torch.nn.Conv1d(out_channels * num_translations, out_channels * num_translations, kernel_size, 
                          padding=kernel_size // 2)
            )
            layers.append(torch.nn.BatchNorm1d(out_channels * num_translations))
            layers.append(torch.nn.ReLU(inplace=True))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.network(x)  # shape: [batch, out_channels * num_translations, signal_length]
        # Global average pooling over the signal dimension
        out = torch.mean(out, dim=-1)  # shape: [batch, out_channels * num_translations]
        batch_size = out.shape[0]
        # Reshape to separate the channels corresponding to each discrete translation
        out = out.view(batch_size, self.out_channels, self.num_translations)
        # Aggregate over the fibre channels to get a per-translation activation vector
        group_activations = torch.mean(out, dim=1)  # shape: [batch, num_translations]
        return group_activations

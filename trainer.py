...
from utils import tsne, mds, _logger, sim_heatmap
import time
...
from data_preprocess import data_preprocess_spectral
...
def setup_dataloaders(args):
    ...
    if args.dataset == 'ptb':
        args.n_feature = 4 # number of channels
        args.len_sw = 1000 # length of time series
        args.n_class = 5 # number of classes
    ...
    if args.dataset == 'epilepsy':
        ...
        args.len_sw = 178
        args.n_class = 2
        train_loaders, val_loader, test_loader = data_preprocess_epilepsy.prep_epilepsy(args)
    if args.dataset == 'spectral':
        args.n_feature = 1
        args.len_sw = 200
        args.n_class = 2
        train_loaders, val_loader, test_loader = data_preprocess_spectral.prep_spectral(args)
...
def setup_model_optm(args, DEVICE, classifier=True):
    ...
    if args.backbone == 'FCN':
        backbone = FCN(n_channels=args.n_feature, n_classes=args.n_class, backbone=True, args=args)
        ...
        backbone = ResNet1D(in_channels=args.n_feature, base_filters=32, kernel_size=5, stride=1, groups=1, n_block=8, n_classes=args.n_class)
    elif args.backbone == 'resnet_ssl':
        backbone = ResNet1D_SSL(in_channels=args.n_feature, base_filters=32, kernel_size=5, stride=1, groups=1, n_block=8, n_classes=args.n_class, covariance_training=args.covariance)
    else:
        NotImplementedError
    ...
    if args.framework in ['byol', 'simsiam']:
        ...
        optimizers = [optimizer]
    elif args.framework == 'auditory':
        model = Auditory(backbone=backbone, out_channels=args.n_feature, DEVICE=DEVICE)
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, betas=(0.9, 0.99), weight_decay=args.weight_decay)
        optimizers = [optimizer]
    else:
        NotImplementedError
...
def setup(args, DEVICE):
    ...
    if args.framework in ['simclr', 'nnclr']:
        args.criterion = 'NTXent' if not args.NTX_mod else 'NTX_modify'
        args.weight_decay = 1e-6
    if args.framework == 'tstcc':
        args.criterion = 'NTXent' if not args.NTX_mod else 'NTX_modify'
        args.backbone = 'FCN'
        args.weight_decay = 3e-4
    if args.framework == 'auditory':
        args.backbone = 'vae'
        args.criterion = 'auditory_criteria'
    ...
    elif args.criterion == 'auditory_criteria':
        criterion = AuditoryLoss(DEVICE, args.batch_size)
    elif args.criterion == 'NTX_modify':
        criterion = NTXentLoss_modify(DEVICE, args.batch_size, temperature=0.1)
...
def calculate_model_loss(args, sample, target, model, criterion, DEVICE, recon=None, nn_replacer=None, view_learner=None):
    # sample --> (Batch_size, time steps, channel size)
    if args.VAE:
        ...
        aug_sample1, aug_sample2 = gen_aug(sample, args.aug1), vanilla_mix_up(STAug2(gen_aug(sample, args.aug2)))
    elif args.CutMix:
    ...
    elif args.robust_shift:
        aug_sample1, aug_sample2 = constant_phase_shift(gen_aug(sample, args.aug1), args, DEVICE), constant_phase_shift(gen_aug(sample, args.aug2), args, DEVICE)
    else:
    ...
    if args.framework in ['tstcc', 'simclr', 'nnclr']:
        assert args.criterion == 'NTXent' or args.criterion == 'NTX_modify'
    if args.framework in ['byol', 'simsiam', 'nnclr']:
        if args.backbone in ['AE', 'CNN_AE']:
            ...
            p1, p2, z1, z2 = model(x1=aug_sample1, x2=aug_sample2, covariance=args.covariance)
        else:
    ...
    if args.framework == 'simclr':
        if args.backbone in ['AE', 'CNN_AE']:
            ...
            z1, z2, loss1, loss_rec = model(x1=aug_sample1, x2=aug_sample2, covariance_training=view_learner)
        else:
        ...
        if args.dim_mixing: # Feature extrapolation
            ...
            loss_contras = criterion(z1, z2)
            return loss_contras+loss1, loss_rec
        else:
            loss = criterion(z1, z2) if not args.NTX_mod else criterion(z1, z2, aug_sample1, aug_sample2)
    ...
    if args.framework == 'auditory':
        z1, z2, z1_rec, z2_rec = model(aug_sample1, aug_sample2)
        loss = criterion(z1, z2, z1_rec, z2_rec, aug_sample1, aug_sample2)
    return loss
...
def train(train_loaders, val_loader, model, DEVICE, optimizers, schedulers, criterion, args):
    ...
    if args.covariance:
        filename = args.save + '_covariance.pth'
        view_learner = cov_rec(z_dim=128, dataset=args.dataset).to(DEVICE)
        optimizer_view_learner = optim.Adam(view_learner.parameters(), lr=0.003) 
    ...
    for epoch in range(args.n_epoch):
        ...
        for i, train_loader in enumerate(train_loaders):
            for idx, (sample, target, domain) in enumerate(train_loader):
                ...
                if args.InfoMin:
                    ...
                    loss, loss_rec = calculate_model_loss(args, sample, target, model, criterion, DEVICE, recon=recon, nn_replacer=nn_replacer, view_learner=view_learner)
                else:
        ...
        if args.cases in ['subject', 'subject_large']:
            with torch.no_grad():
                ...
                # if args.covariance:
                #     torch.save(view_learner.state_dict(), filename)                
        else:
            with torch.no_grad():
                model.eval()
                total_loss = 0
...
def test(test_loader, best_model, DEVICE, criterion, args): # Test the pre-trained model without fine-tuning on the downstream task
...
def train_lincls(train_loaders, val_loader, trained_backbone, classifier, DEVICE, optimizer, criterion, args):
    ...
    # if args.covariance:
    #     trained_view = FCN_controller(n_channels=args.n_feature, args=args).to(DEVICE)
    #     trained_view.load_state_dict(torch.load(args.save + '_covariance.pth'))
    #     optimizer_view_learner = optim.Adam(trained_view.parameters(), lr=0.001) 
    #     trained_view.train()
    ...
    for epoch in range(args.n_epoch):
        ...
        for i, train_loader in enumerate(train_loaders):
            for idx, (sample, target, domain) in enumerate(train_loader):
                #import pdb;pdb.set_trace();
                if args.robust_shift: sample = constant_phase_shift(sample, args, DEVICE)
        ...
        if args.cases in ['subject_large']:
            with torch.no_grad():
                best_lincls = copy.deepcopy(classifier.state_dict())            
        else:
            with torch.no_grad():
...
def test_lincls(test_loader, trained_backbone, best_lincls, DEVICE, criterion, args, plt=False):  # Test the fine-tuned model
    ...
    feats = None
    trgs, preds, otp, cnst = np.array([]), np.array([]), np.array([]), np.array([])
    with torch.no_grad():
        classifier.eval()
        for idx, (sample, target, domain) in enumerate(test_loader):
            ...
            #     ref_frame = trained_view(torch.abs(fftsamples).to(DEVICE).float())
            #     # ref_frame = model_c(sample_framed.to(DEVICE).float())
            #     sample = frame_transform(sample, fftsamples, ref_frame, args, DEVICE)

            sample, target = sample.to(DEVICE).float(), target.to(DEVICE).long()
            loss, predicted, feat, output = calculate_lincls_output(sample, target, trained_backbone, classifier, criterion, args)
            total_loss += loss.item()
            ...
            if args.robust_check:
                ...
                sample_shifted_2 = sample_shifted_2.to(DEVICE).float()
                loss, predicted_shift, feat, output = calculate_lincls_output(sample_shifted_2, target, trained_backbone, classifier, criterion, args)
                cnst = np.append(cnst, (predicted - predicted_shift).data.cpu().numpy())         
        ...
        correlation = f1_score(trgs, preds, average='weighted') * 100 # WF1 score activity recognition

        if args.robust_check: 
            final_const = 100-100*sum((cnst != 0)).item()/(cnst.shape[0])
            final_const = np.mean(np.abs(cnst[(cnst != 0)]))
        else:
            final_const = 0
        ...
        if args.dataset == 'ieee_small' or args.dataset =='ieee_big' or args.dataset == 'dalia':
            ...
            correlation = np.corrcoef(trgs, preds)[0,1] if np.isnan(np.corrcoef(trgs, preds)[0,1]) == False else 0
        if args.dataset == 'ecg':
